<div class="white container-fluid py-5">
    <div class="container">
		<h1>Manipulating the DOM with Prolog</h1>
...
session.answer(); // false</code></pre>
		...
		<ul>
			...
			If the first argument is bound but the second one is not, the second argument will be bound to the first element's parent.
			If the second argument is bound but the first one is not, it will be bound (by reevaluation) to all the children of second argument.
			If both arguments are bound, the predicate checks if the first parameter is a children of the second one.</li>
			<li><a href="http://www.tau-prolog.org/documentation/prolog/dom/sibling/2">sibling/2</a>: if the first argument is bound but the second one is not, it will be bound to the first argument's most immediate right sibling.
			If the first argument is unbound but the second one is not, the first argument will be bound to the second argument's most immediate left sibling. If both arguments are bound, the predicate checks if they are both immediate siblings.</li>
		</ul>
		
		<h2 id="modify" class="mt-5"><a href="#modifier">Modifying the DOM</a></h2>
...
session.answer(); // {Output/&lt;html>(output), Body/&lt;html>(body), Event/&lt;event>(keypress)}</code></pre>
		...
		<h2 id="effects" class="mt-5"><a href="#effects">Effects</a></h2>
